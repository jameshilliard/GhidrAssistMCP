...
/**
 * MCP tool for setting post-instruction comments.
 */
public class SetPostCommentTool implements McpTool {

    @Override
    public String getName() {
        return "set_post_comment";
    }

    @Override
    public String getDescription() {
        return "Set a post-instruction comment (after the line)";
    }
    ...
    public McpSchema.CallToolResult execute(Map<String, Object> arguments, Program currentProgram, GhidrAssistMCPPlugin plugin) {
        ...
        int transactionID = currentProgram.startTransaction("Set post comment");
        ...
        try {
            ...
            return McpSchema.CallToolResult.builder()
                .addTextContent(String.format("Successfully set post comment at %s", addr))
                .build();

        } catch (Exception e) {
            return McpSchema.CallToolResult.builder()
                .addTextContent("Failed to set post comment: " + e.getMessage())
                .build();
        } finally {
