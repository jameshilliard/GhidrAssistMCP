...
/**
 * MCP tool for setting plate (header) comments.
 */
public class SetPlateCommentTool implements McpTool {

    @Override
    public String getName() {
        return "set_plate_comment";
    }

    @Override
    public String getDescription() {
        return "Set a plate (large header) comment at an address";
    }
    ...
    public McpSchema.CallToolResult execute(Map<String, Object> arguments, Program currentProgram, GhidrAssistMCPPlugin plugin) {
        ...
        int transactionID = currentProgram.startTransaction("Set plate comment");
        ...
        try {
            ...
            return McpSchema.CallToolResult.builder()
                .addTextContent(String.format("Successfully set plate comment at %s", addr))
                .build();

        } catch (Exception e) {
            return McpSchema.CallToolResult.builder()
                .addTextContent("Failed to set plate comment: " + e.getMessage())
                .build();
        } finally {
